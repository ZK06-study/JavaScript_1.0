...
// 해설: 
// - 두 개의 매개변수 a, b를 받아서 그 합을 반환하는 간단한 함수
// - 더하기 연산자(+)를 사용하여 두 값을 더함
// - return 키워드로 결과값을 반환
```
...
// 해설:
// - 객체 리터럴을 사용하여 name과 age 속성을 가진 객체 생성
// - introduce 메소드는 템플릿 리터럴을 사용하여 자기소개 문자열 생성
// - this 키워드로 객체의 속성에 접근
// - 메소드 단축 구문을 사용하여 함수 정의
```
...
function findMax(numbers) {
  let max = numbers[0]; // 첫 번째 요소를 초기 최댓값으로 설정
...
// 해설:
// - 배열의 첫 번째 요소를 초기 최댓값으로 설정
// - for 루프로 배열의 나머지 요소들을 순회
// - 각 요소가 현재 최댓값보다 크면 최댓값 갱신
// - Math.max 사용 없이 직접 비교하여 최댓값 찾기
// - 음수가 포함된 배열에서도 올바르게 동작
```